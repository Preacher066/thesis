...
introductory chapter. In Chapter~\nameref{chap:location_nodes}, we first deal with the components of the general problem of data MULE scheduling, and with the aspects that we are concerned, namely, {\em path selection}. Next, we introduce location nodes, location graphs and propose heuristics to compute them. We also provided justification of our choice of heuristic with performance measures. In Chapter~\nameref{chap:steiner}, we describe our main heuristic for path selection and also provide pseudocode for the same. 
